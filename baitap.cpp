...
// Tìm số dương nhỏ nhất trong danh sách
float tim_so_duong_nho_nhat(Node* dau) {
	Node* tam = dau;
	float min_duong = FLT_MAX;  // Khởi tạo min_duong là giá trị cực đại của float
	
	while (tam != NULL) {
		if (tam->du_lieu > 0 && tam->du_lieu < min_duong) {
			min_duong = tam->du_lieu;  // Cập nhật số dương nhỏ nhất
		}
		tam = tam->ke_tiep;
	}
...
int main() {
	...
	float min_duong = tim_so_duong_nho_nhat(dau);
	
	if (min_duong == FLT_MAX) {
...
// Tính tổng các số âm trong danh sách
float tinh_tong_so_am(Node* dau) {
	Node* tam = dau;
	float tong = 0.0;
	
	while (tam != NULL) {
		if (tam->du_lieu < 0) {
			tong += tam->du_lieu;  // Cộng dồn các số âm
		}
		tam = tam->ke_tiep;
	}
...
int main() {
	...
	float tong_am = tinh_tong_so_am(dau);
...
// Ham dem so ky tu 'A' trong danh sach
int dem_ky_tu_A(Node* dau) {
	Node* tam = dau;
	int dem = 0;
	
	while (tam != NULL) {
		if (tam->du_lieu == 'A') {
			dem++;  // Tang bien dem neu ky tu la 'A'
		}
		tam = tam->ke_tiep;
	}
	
	return dem;  // Tra ve so luong ky tu 'A' trong danh sach
}
...
int main() {
	...
	// Dem so ky tu 'A'
	int so_ky_tu_A = dem_ky_tu_A(dau);
	printf("So ky tu 'A' trong danh sach la: %d\n", so_ky_tu_A);
